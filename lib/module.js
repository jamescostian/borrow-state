  ...
  const nextOp = () => {
    if (runningOperations > 0) {
      ...
      state.unblock = state.putBack = once(() => {
        ...
        // If unsafe is true, then don't bother, but otherwise, make sure to do this if you haven't already (if it's readOnly, then the promise chain was cut with a copy, so no need to copy again - they could never mutate the real state)
        if (!unsafe && !readOnly) {
          state = copy(state)
        }
